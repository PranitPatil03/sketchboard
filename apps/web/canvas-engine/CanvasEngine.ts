...
export class CanvasEngine {
  ...
  private onRoomClosed: (() => void) | null;
  ...
  private handleTexty(e: MouseEvent) {
    ...
    textarea.classList.add("sketchboard-texty");
    textarea.style.color = this.strokeFill;
    const fontString = `${calFont}px/1.2 ${this.fontFamily === "normal" ? "Arial" : this.fontFamily === "hand-drawn" ? "Sketchboardfont, Xiaolai" : "Assistant"}`;
    ...
    const sketchboardContainer = document.querySelector(
      ".sketchboard-textEditorContainer"
    );

    if (sketchboardContainer) {
      sketchboardContainer.appendChild(textarea);
    ...
    const save = () => {
      ...
      if (sketchboardContainer?.contains(textarea)) {
        sketchboardContainer.removeChild(textarea);
        if (span && document.body.contains(span)) {
          document.body.removeChild(span);
        }
      }
  ...
  drawText(
    ...
    const fontString = `${fontStyle} ${calFontSize}px/1.2 ${fontFamily === "normal" ? "Arial" : fontFamily === "hand-drawn" ? "Sketchboardfont, Xiaolai" : "Assistant"}`;
  ...
  private handleKeyDown = (e: KeyboardEvent) => {
    if ((e.ctrlKey || e.metaKey) && e.key === "a") {
      ...
      if (this.existingShapes.length > 0) {
        ...
        this.SelectionController.selectAll(this.existingShapes);
        this.clearCanvas();
      }
      return;
    }
    ...
    if (e.key === "Delete" || e.key === "Backspace") {
      if (this.activeTool === "selection") {
        if (this.SelectionController.hasMultiSelection()) {
          const shapesToDelete = this.SelectionController.getSelectedShapes();
