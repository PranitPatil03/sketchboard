...
export default function CanvasBoard() {
    ...
    useEffect(() => {
        ...
        const updateRoomParams = async () => {
            if (isPending) return;
            const hash = getHash();
            currentHashRef.current = hash;
            const currentRoomParams = getRoomParamsFromHash(hash);

            if (session?.user && currentRoomParams) {
                const result = await joinRoom({ id: currentRoomParams.roomId });
                if (!result.success) {
                    toast({ title: result.error || "Failed to join room", variant: "destructive" });
                    setMode("standalone");
                    return;
                }
                setMode("room");
                setIsRoomAdmin(result.room?.adminId === session.user.id);
                userRef.current = {
                    roomId: currentRoomParams.roomId,
                    encryptionKey: currentRoomParams.encryptionKey,
                    userId: session.user.id ?? null,
                    userName: session.user.name ?? null,
                    token: document.cookie.split('; ').find(c => c.startsWith('accessToken='))?.split('=')[1] ?? null,
                };
            } else if (!session?.user && currentRoomParams) {
                toast({ title: "You need to be signed in to join a collaborative room. Redirecting to sign in...", variant: "destructive" });
                setMode("standalone");
                router.push(`/auth/signin?callbackUrl=${encodeURIComponent(hash)}`);
            } else {
                setMode("standalone");
    ...
    }, [pathname, searchParams, isPending, session, router]);
    ...
    const initializeCanvasEngine = useCallback(() => {
        ...
        const engine = new CanvasEngine(
            ...
            mode === 'room' ? handleRoomClosed : null
        );
        ...
        return engine;
    }, [canvasEngineState.canvasColor, mode, theme, handleRoomClosed]);
    ...
    return (
        <div className={cn("sketchboard h-screen overflow-hidden",
            ...
            <canvas className={cn("sketchboard sketchboard-canvas touch-none", theme === 'dark' ? 'sketchboard-canvas-dark' : '')} ref={canvasRef} />
        </div >
    )
};