                ...
                <div className={cn("flex flex-col", isMobile ? "" : "h-[calc(100vh-135px)] Island rounded-lg")}>
                    <div className={cn("py-1", isMobile ? "" : "flex-1 overflow-auto py-1 custom-scrollbar")}>
                        <nav className={cn("grid gap-1", isMobile ? "px-0" : "px-2")}>
                            <SidebarItem icon={Command} label="Command palette" shortcut="Ctrl+/" onClick={() => setCommandPaletteOpen(true)} />
                                ...
                                <>
                                    ...
                                    {participants && participants.length > 0 && (
                                        <div className="px-3 py-2">
                                            <p className="text-xs font-medium text-muted-foreground mb-2">Participants ({participants.length})</p>
                                            <div className="flex flex-wrap gap-1.5">
                                                {participants.map((p) => (
                                                    <div
                                                        key={p.userId}
                                                        title={p.userName}
                                                        style={{ backgroundColor: getClientColor(p) }}
                                                        className="w-7 h-7 rounded-full flex items-center justify-center cursor-default"
                                                    >
                                                        <span className="text-xs font-bold text-gray-900">{p.userName.charAt(0).toUpperCase()}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                            ...
                            {session?.user && session?.user.id ? (
                                <SidebarItem icon={LogOut} label="Log Out" onClick={async () => {
                                    await signOut();
